...
To predict new samples, \code{predict.train} can be used. For classification models, the default behavior is to calculate the predicted class. Using the option \code{type = "prob"} can be used to compute class probabilities from the model. For example:
<<plsPred>>=
